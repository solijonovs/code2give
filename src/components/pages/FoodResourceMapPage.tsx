...
export function FoodResourceMapPage() {
  ...
  return (
    <div className="relative overflow-hidden" style={{ height: "calc(100vh - 64px)" }}>
      ...
      {/* ── B. Left sliding drawer ──────────────────────────────────────────── */}
      <div className={`absolute left-0 top-0 h-full w-80 bg-white shadow-2xl flex flex-col transition-transform duration-200 z-20 ${
        ...
        <div className="px-4 py-3 bg-white border-b border-gray-200 shrink-0">
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-1.5 flex-1">
              <div className="flex items-center gap-1.5 bg-gray-100 rounded-lg px-2.5 py-1.5 flex-1">
      ...
      {/* B2. Tab to reopen drawer when closed */}
      {!listOpen && (
        <button onClick={() => setListOpen(true)}
          className="absolute left-0 top-1/2 -translate-y-1/2 z-20 bg-white rounded-r-lg shadow-lg px-2 py-3 hover:bg-gray-50 transition-colors">
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            <div className="ml-auto flex items-center gap-1.5">
              ...
              <select onChange={e => { if (e.target.value) { handleTagClick(e.target.value); e.currentTarget.value = ""; } }}
                className="border border-gray-200 rounded-lg px-2 py-1 text-xs bg-white focus:outline-none focus:ring-2 focus:ring-primary/30 cursor-pointer max-w-40">
              ...
              <Button variant="ghost" size="sm" onClick={() => exportToCSV(filteredResources, "food-resources.csv")} className="h-8 gap-1.5 text-gray-600">
      ...
      {/* ── D. Legend (floating top-right) ──────────────────────────────────── */}
      <div className="absolute top-3 right-3 z-10 bg-white/95 backdrop-blur-sm rounded-xl shadow-md border border-gray-200 p-3 min-w-38">
